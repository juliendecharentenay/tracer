...
- [ ] **Canvas background**
  ...
  Display the cropped image as the background of a canvas area that fills most of the viewport. The image should be rendered at its natural dimensions as a non-interactive background layer. This canvas area is the container for all subsequent SVG overlay work.

  **Success criteria:** after cropping, the cropped image is visible as the full-area background with no crop overlay present.

### 6. SVG canvas outline overlay
- [ ] **SVG canvas outline overlay**

  Render a visible rectangular outline (an `<svg>` element with a stroke border) overlaid on the background image. The outline represents the final SVG canvas boundary. It is initialised to a sensible default size (e.g. 80 % of the image dimensions) centred on the image.

  **Success criteria:** a visible rectangle outline appears over the background image when the canvas-setup phase is entered.

### 7. Canvas outline — reposition and resize
- [ ] **Canvas outline — reposition and resize**

  Allow the user to drag the SVG canvas outline to reposition it over the background image, and to resize it by dragging its edges or corners. The outline must remain fully within the image bounds during interaction. Implement as a composable (e.g. `useDraggableRect`) with unit tests for the position/size clamping logic.

  **Success criteria:** the user can drag the canvas outline to a new position and resize it; it cannot be dragged outside the image area.

### 8. ViewBox configuration
- [ ] **ViewBox configuration**

  Provide numeric inputs for the SVG `viewBox` width and height. When either value changes, update the canvas outline's aspect ratio to match the chosen viewBox dimensions (the outline's screen size adjusts while keeping its top-left corner fixed or centred, as appropriate). Store the chosen width and height in canvas state.

  **Success criteria:** changing the viewBox width or height adjusts the outline's aspect ratio on screen immediately.

### 9. Approve canvas configuration
- [ ] **Approve canvas configuration**

  Add a button to confirm the canvas configuration and proceed to the tracing phase. Clicking it locks in the canvas outline position, size, and viewBox values, and switches the application to tracing mode. The background image and canvas outline remain visible; no further repositioning or viewBox editing is available.

  **Success criteria:** after clicking "Approve", the canvas configuration controls disappear and the tracing interface becomes active.

---

## Phase 4 — Core data structure and utilities

### 10. SVG data structure and point deduplication
...
### 12. Draw a straight line — start point
...
### 13. Draw a straight line — live preview
...
### 14. Draw a straight line — end point and commit
...
### 20. Path selection — click behaviour
...
### 22. Draggable point handles (selected path)
...
### 24. Toggle line ↔ cubic Bézier (double click)
...
### 25. Control point symbols and drag (cubic Bézier)
...
### 27. Restore state from localStorage
